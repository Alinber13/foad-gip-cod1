/* =====================================================================
   FOAD COD1 — Fonction serverless Vercel (catch-all /api/*)
   GIP Aviation / BA 125 Istres
   Stockage : Upstash Redis / Vercel KV (REST) si configuré,
   sinon repli en mémoire (non durable — connecter une base KV).
   Auth admin : token déterministe (HMAC du mot de passe) -> stateless.
   ===================================================================== */
...
/* -------- mémoire de repli (par instance, non durable) -------- */
let MEM = null;
...
async function kvGet(key) {
  ...
  return j.result; // string ou null
}
...
async function loadDB() {
  if (KV_URL && KV_TOKEN) {
    const s = await kvGet(DB_KEY);
    return s ? JSON.parse(s) : { stagiaires: [] };
  }
...
module.exports = async (req, res) => {
  try {
    const seg = [].concat(req.query && req.query.path ? req.query.path : []);
    // repli : déduire le chemin depuis l'URL si query.path absent
    let parts = seg;
    if (!parts.length) {
    ...
    if (route === 'register' && method === 'POST') {
      ...
      if (!b.nom || !b.prenom || !b.matricule)
        return send(res, 400, { error: 'Nom, prénom et matricule sont obligatoires.' });
